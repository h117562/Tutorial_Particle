...
bool EffectShaderClass::Initialize(ID3D11Device* pDevice, HWND hwnd)
{
	...
	//매트릭스 버퍼 설정
	matrixBufferDesc.ByteWidth = sizeof(MatrixBufferType);//버퍼 크기
	...
	//매트릭스 버퍼 설정
	noiseBufferDesc.ByteWidth = sizeof(NoiseBufferType);//버퍼 크기
	noiseBufferDesc.Usage = D3D11_USAGE_DYNAMIC;//GPU(읽기 전용)와 CPU(쓰기 전용)에서 액세스 가능 (매 프레임 마다 업데이트)
	noiseBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;//버퍼를 상수 버퍼로 바인딩
	noiseBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;//CPU 액세스 유형
	noiseBufferDesc.MiscFlags = 0;//리소스에 대한 플래그
